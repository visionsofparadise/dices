 ...
 * @throws {DicesOverlayError} If any validation fails
 *
 * @example
 * ```typescript
...
export const verifyEnvelope = (envelope: Envelope, remoteNodeId: Uint8Array): void => {
	...
	if (envelope.version !== Envelope.PROTOCOL_VERSION) {
		throw new DicesOverlayError(`Unsupported protocol version: ${envelope.version}, expected ${Envelope.PROTOCOL_VERSION}`);
	}

	// Validate keyId length (8 bytes)
	if (envelope.keyId.byteLength !== 8) {
		throw new DicesOverlayError(`Invalid keyId length: ${envelope.keyId.byteLength}, expected 8`);
	}

	// Validate dhPublicKey length (32 bytes for X25519)
	if (envelope.dhPublicKey.byteLength !== 32) {
		throw new DicesOverlayError(`Invalid dhPublicKey length: ${envelope.dhPublicKey.byteLength}, expected 32`);
	}
	...
	for (const lowOrderPoint of LOW_ORDER_POINTS) {
		if (compare(envelope.dhPublicKey, lowOrderPoint) === 0) {
			throw new DicesOverlayError("Invalid X25519 public key: low-order point detected");
		}
	}

	// Validate messageNumber (non-negative, safe integer)
	if (envelope.messageNumber < 0 || !Number.isSafeInteger(envelope.messageNumber)) {
		throw new DicesOverlayError(`Invalid messageNumber: ${envelope.messageNumber}`);
	}
	...
	if (envelope.previousChainLength < 0 || !Number.isSafeInteger(envelope.previousChainLength)) {
		throw new DicesOverlayError(`Invalid previousChainLength: ${envelope.previousChainLength}`);
	}
	...
	if (envelope.kemCiphertext && envelope.kemCiphertext.byteLength !== MlKemCipherTextCodec.byteLength()) {
		throw new DicesOverlayError(`Invalid kemCiphertext length: ${envelope.kemCiphertext.byteLength}, expected ${MlKemCipherTextCodec.byteLength()}`);
	}
