...
export class Overlay {
	 ...
	 * routing table, and database, aborts all pending response listeners, and emits 'close' event.
	 * Returns a promise that resolves when the database is fully closed.
	 *
	 * @returns Promise that resolves when overlay is fully closed
	 *
	 * @example
	 * ```typescript
	 * await overlay.close();
	...
	close = async (): Promise<void> => {
		if (this.state === Overlay.STATE.CLOSED) return;
		...
		this.diceClient.events.removeListener("diceAddress", this.diceClientListeners.diceAddressListener);
		this.diceClient.events.removeListener("message", this.diceClientListeners.messageListener);

		this.events.removeListener("error", this.clientListeners.errorListener);
		this.events.removeListener("message", this.clientListeners.messageListener);

		this.diceClient.close();
		this.nodes.close();

		for (const { abort } of this.responseListenerMap.values()) abort.abort("close");
		this.responseListenerMap.clear();

		// Close database to release resources and prevent leaks
		await this.database.close();

		this.state = Overlay.STATE.CLOSED;
	...
	healthcheck = async (): Promise<void> => {
		try {
			...
			for (const node of this.nodes.table) {
				const isRecentlyContacted = node.lastContactedAt && now - node.lastContactedAt < this.options.healthcheckIntervalMs;
			...
			const isAllMarkedError = this.nodes.table.buckets.every((bucket) => bucket.items.every((item) => item.errorCount > 0));

			if (isAllMarkedError || !this.nodes.table.length) {
				await this.findNode(this.keys.nodeId);
				await this.findNode(crypto.getRandomValues(new Uint8Array(20)));
			}
		} catch (error) {
	 ...
	 * ```
	 */
	open = async (isBootstrapping = true): Promise<void> => {
		...
		this.diceClient.events.on("diceAddress", this.diceClientListeners.diceAddressListener);
		this.diceClient.events.on("message", this.diceClientListeners.messageListener);
		await this.diceClient.open(isBootstrapping);

		this.events.on("error", this.clientListeners.errorListener);
		this.events.on("message", this.clientListeners.messageListener);

		this.currentRatchetKeys = await this.loadRatchetKeys();
		this.value = Value.create(
			{
				initiationKeys: this.currentRatchetKeys,
			},
			this.keys
		);

		this.healthcheckInterval = setInterval(() => this.healthcheck(), this.options.healthcheckIntervalMs);
		this.pruneInterval = setInterval(() => this.prune(), this.options.pruneIntervalMs);
	...
	/**
	 * Returns the number of pending response listeners.
	 *
	 * Useful for monitoring and debugging pending requests. A high count may indicate
	 * timeouts, network issues, or resource leaks.
	 */
	get pendingRequestCount(): number {
		return this.responseListenerMap.size;
	}
}
